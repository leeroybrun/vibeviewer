...
- **成本对比洞察**：在设置中配置订阅价格与模型自定义，估算直接调用各家 API 的成本并与 Cursor 订阅价值对比。
...
├─ Packages/
│  ├─ AIUsageTrackerCore/               # Core：工具/扩展/通用服务（不依赖业务层）
│  ├─ AIUsageTrackerModel/              # Model：纯领域实体（值类型/Sendable）
│  ├─ AIUsageTrackerAPI/                # API：网络/IO + DTO→Domain 映射（对外仅暴露服务协议）
│  ├─ AIUsageTrackerAppEnvironment/     # 环境注入与跨特性服务入口
│  ├─ AIUsageTrackerStorage/            # 存储（设置、凭据等）
│  ├─ AIUsageTrackerLoginUI/            # Feature：登录相关 UI
│  ├─ AIUsageTrackerMenuUI/             # Feature：菜单栏弹窗 UI（主界面）
│  ├─ AIUsageTrackerSettingsUI/         # Feature：设置界面 UI
│  └─ AIUsageTrackerShareUI/            # Feature：分享组件与资源
